...
# This script will run every 5 minutes via crontab scheduling. It will check if each container has run for a random duration of time between 45 minutes and 60 minutes (inclusive). This script will be run in the crontab and will run the recycling_script script within it in order to check each honeypot to see if the honeypot should be recycled (deleted and reconfigured) if the duration of time randomly chosen has passed. 
