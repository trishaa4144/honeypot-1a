...
# This script will take a number of minutes to run the container, the public IP address, and the assigned container name as arguments. The script will then check to see if a container is running and for how long it has been running. If the container has been running for the specified amount of time, then the script will delete the networking rules between the container, public IP, and MITM server and then destroy the container. It will immediately call itself (the script) again with the randomized minute value passed in to restart the container with a new random honey & expiration time. Otherwise, if the container is not running at all (meaning the container was destroyed), create the container and establish the network rules between the container, the public IP, and MITM server. However, if the container is running but not for the allotted time, then the script will simply notify the user that the container is not ready to be recycled.
