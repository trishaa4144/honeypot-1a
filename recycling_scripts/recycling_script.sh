...
if [[ -e time_$container_name ]]; then
  ...
  sudo lxc-attach -n "$container_name" -- bash -c `sudo wget -O /tmp/netdata-kickstart.sh https://my-netdata.io/kickstart.sh && sudo sh /tmp/netdata-kickstart.sh --dont-wait --nightly-channel --claim-token $NETDATA_CLAIM_TOKEN --claim-rooms $NETDATA_CLAIM_ROOMS --claim-url https://app.netdata.cloud`
