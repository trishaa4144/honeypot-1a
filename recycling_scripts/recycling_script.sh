...
if [[ -e time_$container_name ]]; then
  ...
  sudo forever -l ~/mitm_logs/"$container_name"\_"$date".log start ~/MITM/mitm.js -n "$container_name" -i "$container_ip" -p 10123 --auto-access --auto-access-fixed 3 --debug  
  ...
  sudo iptables --table nat --insert PREROUTING --source 0.0.0.0/0 --destination "$ext_ip" --protocol tcp --dport 22 --jump DNAT --to-destination "127.0.0.1:10123"
  exit 0
fi
