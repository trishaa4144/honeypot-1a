...
if [[ -e time_$container_name ]]; then
  ...
  sudo forever -l ~/mitm_logs/"$honey_type"/"$container_name"\_"$date".log start ~/MITM/mitm.js -n "$container_name" -i "$container_ip" -p 4567 --auto-access --auto-access-fixed 3 --debug
